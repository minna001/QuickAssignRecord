////<reference path="../../../../node_modules/@types/jqueryui/index.d.ts"/>
import {IInputs, IOutputs} from "./generated/ManifestTypes";
import * as $ from "jquery";

export class PCFAssignRecord implements ComponentFramework.StandardControl<IInputs, IOutputs> {

	private _div : HTMLDivElement;
	private _input: HTMLInputElement;
	private _dataList : HTMLDataListElement;
	private _submitButton : HTMLElement;
	private _submitClicked: EventListenerOrEventListenerObject;
	private _context: ComponentFramework.Context<IInputs>;
    private _notifyOutputChanged: () => void;
	private _container: HTMLDivElement;
	private _Generate : EventListenerOrEventListenerObject;

	/**
	 * Empty constructor.
	 */
	constructor()
	{

	}

	/**
	 * Used to initialize the control instance. Controls can kick off remote server calls and other initialization actions here.
	 * Data-set values are not initialized here, use updateView.
	 * @param context The entire property bag available to control via Context Object; It contains values as set up by the customizer mapped to property names defined in the manifest, as well as utility functions.
	 * @param notifyOutputChanged A callback method to alert the framework that the control has new outputs ready to be retrieved asynchronously.
	 * @param state A piece of data that persists in one session for a single user. Can be set at any point in a controls life cycle by calling 'setControlState' in the Mode interface.
	 * @param container If a control is marked control-type='standard', it will receive an empty div element within which it can render its content.
	 */
	public init(context: ComponentFramework.Context<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container:HTMLDivElement)
	{
		// Add control initialization code
		this._context = context;
        this._notifyOutputChanged = notifyOutputChanged;
		this._container = container;
		this._submitClicked = this.submitClick.bind(this);

		this._input = document.createElement("input");
        this._input.setAttribute("type", "text");
		this._input.setAttribute("id", "query");  
		this._input.setAttribute("class","text-field valid");
		this._input.setAttribute("autocomplete","off");
		this._input.setAttribute("placeholder", "Enter assignee name"); 
		this._input.setAttribute("list", "anrede"); 
		this._input.setAttribute("style", "width:200px");

		this._dataList = document.createElement("datalist");
		this._dataList.setAttribute("id", "anrede"); 

		this._div = document.createElement("div");
		this._div.setAttribute("id", "main"); 

        

		// submit button 
        this._submitButton = document.createElement("input");
        this._submitButton.setAttribute("type", "button");
        this._submitButton.setAttribute("value", "Assign");
		this._submitButton.addEventListener("click", this._submitClicked);

		this._div.appendChild(this._input);
		this._div.appendChild(this._dataList);
		this._div.appendChild(this._submitButton);
		
		this._container.appendChild(this._div);

	

		//var myUsers = new Array();
		//var userLookup = new Array();
		
		
		var fetchXML = `<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
		<entity name='systemuser'>
		  <attribute name='fullname' />
		  <attribute name='systemuserid' />
		  <order attribute='fullname' descending='false' />
		  <filter type='and'>
			<condition attribute='isdisabled' operator='eq' value='0' />
			<condition attribute='accessmode' operator='eq' value='0' />
		  </filter>
		</entity>
	  </fetch>`;

		fetchXML = "?fetchXml="+encodeURIComponent(fetchXML);

		// @ts-ignore
		Xrm.WebApi.retrieveMultipleRecords("systemuser",fetchXML).then
		(function success(result){
			for(var i=0;i<result.entities.length;i++)
			{
				var option = $('<option value="'+result.entities[i].fullname+'" data-value="'+result.entities[i].systemuserid+'"></option>');
				$('#anrede').append(option);
			}
		},
		function (error) {alert(error.message);});
	}

	public submitClick(evt: Event): void {
		var shownVal = (<HTMLInputElement>document.getElementById("query")).value;
		var value2send = (<HTMLInputElement>document.querySelector("#anrede option[value='"+shownVal+"']")).dataset.value;

		//@ts-ignore
		Xrm.Page.getAttribute("ownerid").setValue([{ id: value2send, name: shownVal, entityType: "systemuser" }]);
		//@ts-ignore
		Xrm.Page.data.save();

	}


	/**
	 * Called when any value in the property bag has changed. This includes field values, data-sets, global values such as container height and width, offline status, control metadata values such as label, visible, etc.
	 * @param context The entire property bag available to control via Context Object; It contains values as set up by the customizer mapped to names defined in the manifest, as well as utility functions
	 */
	public updateView(context: ComponentFramework.Context<IInputs>): void
	{
		// Add code to update control view
	}

	/** 
	 * It is called by the framework prior to a control receiving new data. 
	 * @returns an object based on nomenclature defined in manifest, expecting object[s] for property marked as “bound” or “output”
	 */
	public getOutputs(): IOutputs
	{
		return {};
	}

	/** 
	 * Called when the control is to be removed from the DOM tree. Controls should use this call for cleanup.
	 * i.e. cancelling any pending remote calls, removing listeners, etc.
	 */
	public destroy(): void
	{
		// Add code to cleanup control if necessary
	}
}